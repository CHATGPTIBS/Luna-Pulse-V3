...
class Settings:
    ...
    # Standalone Solana meme-coin paper bot
    meme_bot_scan_seconds: int = int(os.getenv("LUNA_MEME_SCAN_SECONDS", "60"))
    meme_bot_candidate_limit: int = int(os.getenv("LUNA_MEME_CANDIDATES", "60"))
    meme_bot_max_positions: int = int(os.getenv("LUNA_MEME_MAX_POSITIONS", "4"))
    meme_bot_risk_pct: float = float(os.getenv("LUNA_MEME_RISK_PCT", "0.002"))
    meme_bot_max_total_risk_pct: float = float(os.getenv("LUNA_MEME_MAX_TOTAL_RISK_PCT", "0.01"))
    meme_bot_entry_score: float = float(os.getenv("LUNA_MEME_ENTRY_SCORE", "7.8"))
    meme_bot_reward_risk: float = float(os.getenv("LUNA_MEME_REWARD_RISK", "2.0"))
    meme_bot_min_liquidity: float = float(os.getenv("LUNA_MEME_MIN_LIQUIDITY", "150000"))
    meme_bot_min_h1_volume: float = float(os.getenv("LUNA_MEME_MIN_H1_VOLUME", "50000"))
    meme_bot_min_m5_volume: float = float(os.getenv("LUNA_MEME_MIN_M5_VOLUME", "5000"))
    meme_bot_min_pair_age_minutes: int = int(os.getenv("LUNA_MEME_MIN_AGE_MINUTES", "20"))
    meme_bot_cooldown_minutes: int = int(os.getenv("LUNA_MEME_COOLDOWN_MINUTES", "60"))
    meme_bot_fee_rate: float = float(os.getenv("LUNA_MEME_FEE_RATE", "0.005"))
    meme_bot_slippage_rate: float = float(os.getenv("LUNA_MEME_SLIPPAGE_RATE", "0.0035"))
