...
def init_db(starting_balance: float):
    with connect() as con:
        ...
        CREATE TABLE IF NOT EXISTS user_accounts (
            ...
            meme_auto_enabled INTEGER NOT NULL DEFAULT 0,
            updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
            FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE
        );
        ...
        if not _has_column(con, "user_accounts", "meme_auto_enabled"):
            con.execute("ALTER TABLE user_accounts ADD COLUMN meme_auto_enabled INTEGER NOT NULL DEFAULT 0")
...
def set_meme_auto(user_id: int, enabled: bool):
    with connect() as con:
        con.execute(
            "UPDATE user_accounts SET meme_auto_enabled=?,updated_at=CURRENT_TIMESTAMP WHERE user_id=?",
            (1 if enabled else 0, user_id),
        )


def list_meme_auto_user_ids() -> list[int]:
    with connect() as con:
        return [int(r["user_id"]) for r in con.execute(
            "SELECT user_id FROM user_accounts WHERE meme_auto_enabled=1"
        ).fetchall()]


def list_meme_positions(user_id: int | None = None) -> list[dict]:
    with connect() as con:
        if user_id is None:
            rows = con.execute("SELECT * FROM meme_positions ORDER BY opened_at DESC").fetchall()
        else:
            rows = con.execute(
                "SELECT * FROM meme_positions WHERE user_id=? ORDER BY opened_at DESC", (user_id,)
            ).fetchall()
        return [dict(r) for r in rows]


def add_meme_position(user_id: int, p: dict):
    with connect() as con:
        cur = con.execute("""
            INSERT INTO meme_positions(
              user_id,token_address,pair_address,dex_id,symbol,name,entry,stop,target,
              qty,risk_cash,score,peak_price,source,reason
            ) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
        """, (
            user_id, p["token_address"], p.get("pair_address", ""), p.get("dex_id", ""),
            p["symbol"], p.get("name", p["symbol"]), p["entry"], p["stop"], p["target"],
            p["qty"], p["risk_cash"], p["score"], p["peak_price"], p["source"], p.get("reason", ""),
        ))
        return cur.lastrowid


def update_meme_peak(position_id: int, peak_price: float):
    with connect() as con:
        con.execute(
            "UPDATE meme_positions SET peak_price=MAX(peak_price,?) WHERE id=?",
            (peak_price, position_id),
        )


def update_meme_stop(position_id: int, stop: float):
    with connect() as con:
        con.execute("UPDATE meme_positions SET stop=? WHERE id=?", (stop, position_id))


def close_meme_position(position_id: int, exit_price: float, pnl: float, fees: float, reason: str):
    with connect() as con:
        p = con.execute("SELECT * FROM meme_positions WHERE id=?", (position_id,)).fetchone()
        if not p:
            return False
        p = dict(p)
        con.execute("""
            INSERT INTO meme_trades(
              user_id,token_address,pair_address,dex_id,symbol,name,entry,exit,qty,pnl,
              fees,reason,entry_score,source,opened_at
            ) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
        """, (
            p["user_id"], p["token_address"], p["pair_address"], p["dex_id"], p["symbol"],
            p["name"], p["entry"], exit_price, p["qty"], pnl, fees, reason, p["score"],
            p["source"], p["opened_at"],
        ))
        con.execute("DELETE FROM meme_positions WHERE id=?", (position_id,))
        con.execute(
            "UPDATE user_accounts SET cash=cash+?,updated_at=CURRENT_TIMESTAMP WHERE user_id=?",
            (pnl, p["user_id"]),
        )
...
def list_meme_trades(user_id: int, limit: int = 200):
    with connect() as con:
        return [dict(r) for r in con.execute(
            "SELECT * FROM meme_trades WHERE user_id=? ORDER BY closed_at DESC LIMIT ?",
            (user_id, limit),
        ).fetchall()]


def meme_recent_stop(user_id: int, token_address: str, cooldown_minutes: int) -> bool:
    with connect() as con:
        row = con.execute("""
            SELECT 1 FROM meme_trades
            WHERE user_id=? AND token_address=? AND reason='stop'
              AND closed_at >= datetime('now', ?)
            LIMIT 1
        """, (user_id, token_address, f"-{int(cooldown_minutes)} minutes")).fetchone()
        return bool(row)
...
def reset_user_account(user_id: int, starting_balance: float):
    with connect() as con:
        ...
        con.execute("DELETE FROM meme_positions WHERE user_id=?", (user_id,))
        con.execute("DELETE FROM meme_trades WHERE user_id=?", (user_id,))
        con.execute(
            """UPDATE user_accounts SET starting_balance=?,cash=?,
               crypto_auto_enabled=0,forex_auto_enabled=0,meme_auto_enabled=0,updated_at=CURRENT_TIMESTAMP
               WHERE user_id=?""",
