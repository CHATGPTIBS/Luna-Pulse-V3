...
export async function executeBuy({ store, mint, solAmount, mode = 'paper', source = MANUAL, automated = false }) {
  ...
  if (m === 'paper') {
    return {
      mode: m,
      ...(await paperBuy({ store, mint, solAmount: amount, leaderAddress: source, leaderLabel: source === MANUAL ? 'Manual' : 'Strategy' })),
    };
  }
  ...
  pushLiveHistory(store, { side: 'BUY', mint, solAmount: r.solAmount, tokenAmount: null, priceUsd: r.quality?.tokenPriceUsd || null, signature, realizedPnlSol: 0, source });
  return { mode: m, solAmount: r.solAmount, signature, info: r.info, quality: r.quality };
}
...
export async function executeSell({ store, mint, percent = 100, mode = 'paper', source = MANUAL, automated = false, leaderAddress = null }) {
  ...
  if (m === 'paper') {
    try {
      return {
        mode: m,
        ...(await paperSell({ store, mint, sellPct: pct / 100, leaderAddress, leaderLabel: source === MANUAL ? 'Manual' : 'Strategy' })),
      };
    } catch (e) {
      // A manual sell may intentionally close an existing copied/strategy paper
      // position. Automated exits never fall back to another leader's position.
      if (source !== MANUAL || !leaderAddress || !/No paper position/i.test(String(e.message || e))) throw e;
      return { mode: m, ...(await paperSell({ store, mint, sellPct: pct / 100, leaderAddress: null, leaderLabel: 'Manual' })) };
    }
  }
  ...
  const r = await copySell({ mint, rawAmount: raw, wallet });
  const signature = r.result?.signature || r.result?.txid || null;
  pushLiveHistory(store, { side: 'SELL', mint, solAmount: null, tokenAmount: null, priceUsd: null, signature, realizedPnlSol: null, source });
  return { mode: m, percent: pct, rawAmount: raw.toString(), signature };
}
...
export async function createLimitOrder({ store, mint, side, direction, targetType = 'price', target, solAmount = null, sellPct = null, mode = 'paper' }) {
  ...
  const order = {
    id: store.data.nextOrderId++,
    kind: 'limit', side, mint, mode: m, status: 'open', direction: dir, targetType: type, target: value,
    solAmount: side === 'buy' ? Number(solAmount) : null,
    sellPct: side === 'sell' ? clampPct(sellPct) : null,
    createdAt: nowIso(), basePriceUsd: intel.priceUsd, baseMarketCapUsd: intel.marketCapUsd || 0, executions: 0,
  };
...
export async function createDcaOrder({ store, mint, side, solAmount = null, sellPct = null, intervalSec, runs, mode = 'paper' }) {
  ...
  const order = {
    id: store.data.nextOrderId++, kind: 'dca', side, mint, mode: m, status: 'open',
    solAmount: side === 'buy' ? Number(solAmount) : null,
    sellPct: side === 'sell' ? clampPct(sellPct) : null,
    intervalSec: interval, runsTotal: count, runsRemaining: count, nextRunAt: Date.now(), executions: 0, createdAt: nowIso(),
  };
  store.data.orders.push(order); store.save(); return order;
}
...
export async function createAutoSellSet({ store, mint, takeProfitPct = 0, stopLossPct = 0, trailingPct = 0, sellPct = 100, mode = 'paper', leaderAddress = null }) {
  ...
  const sl = Math.abs(Number(stopLossPct || 0));
  const trail = Math.abs(Number(trailingPct || 0));
  if (tp > 0) created.push({
    id: store.data.nextOrderId++, kind: 'take-profit', side: 'sell', mint, mode: m, status: 'open', direction: 'above', targetType: 'price',
    target: intel.priceUsd * (1 + tp / 100), sellPct: clampPct(sellPct), basePriceUsd: intel.priceUsd, groupId, leaderAddress, createdAt: nowIso(), executions: 0,
  });
  if (sl > 0) created.push({
    id: store.data.nextOrderId++, kind: 'stop-loss', side: 'sell', mint, mode: m, status: 'open', direction: 'below', targetType: 'price',
    target: intel.priceUsd * (1 - sl / 100), sellPct: clampPct(sellPct), basePriceUsd: intel.priceUsd, groupId, leaderAddress, createdAt: nowIso(), executions: 0,
  });
  if (trail > 0) created.push({
    id: store.data.nextOrderId++, kind: 'trailing-stop', side: 'sell', mint, mode: m, status: 'open',
    trailPct: trail, sellPct: clampPct(sellPct), basePriceUsd: intel.priceUsd, highWaterPriceUsd: intel.priceUsd, groupId, leaderAddress, createdAt: nowIso(), executions: 0,
  });
...
export function listOrders(store, includeClosed = false) {
  const rows = includeClosed ? store.data.orders : store.data.orders.filter(activeOrder);
...
export async function createSniper({ store, label, solAmount, minLiquidityUsd = 0, maxMarketCapUsd = 0, maxSnipes = 1, mode = 'paper' }) {
  ...
  const setup = {
    id: store.data.nextSniperId++, label: label || `Sniper ${store.data.nextSniperId - 1}`, mode: m, enabled: true,
    solAmount: Number(solAmount || 0), minLiquidityUsd: Number(minLiquidityUsd || 0), maxMarketCapUsd: Number(maxMarketCapUsd || 0),
    maxSnipes: Math.max(1, Math.min(100, Number(maxSnipes || 1))), completed: 0, createdAt: nowIso(),
...
export function editSniperState(store, id, action) {
  ...
  if (!s) return null;
  if (action === 'remove') store.data.snipers = store.data.snipers.filter(x => x !== s);
  else s.enabled = action === 'resume';
  store.save(); return s;
}

async function targetHit(order, intel) {
...
async function executeOrder(client, store, order) {
  try {
    ...
    if (order.kind === 'dca') {
      ...
      order.status = 'filled'; order.closedAt = nowIso();
      if (order.groupId) {
        for (const sibling of store.data.orders) {
          if (sibling !== order && sibling.groupId === order.groupId && sibling.status === 'open') {
            sibling.status = 'cancelled'; sibling.closedAt = nowIso(); sibling.cancelReason = `Sibling #${order.id} filled`;
          }
        }
      }
    }
    order.lastError = null; store.save();
    await notify(client, store, `⚙️ **V4 order #${order.id} filled** · ${order.side.toUpperCase()} \`${order.mint}\` · ${order.mode.toUpperCase()}${result?.signature ? `\nhttps://solscan.io/tx/${result.signature}` : ''}`);
  } catch (e) {
    ...
    store.save();
    if (order.status === 'failed') await notify(client, store, `⚠️ **V4 order #${order.id} failed** after 3 attempts: ${order.lastError}`);
  }
}

async function processOrders(client, store) {
  if (store.data.settings.paused) return;
  const open = store.data.orders.filter(activeOrder);
  for (const order of open) {
    if (order.kind === 'dca') {
      if (Date.now() >= Number(order.nextRunAt || 0)) { await executeOrder(client, store, order); return; }
      continue;
    }
    let intel;
    try { intel = await getTokenIntel(order.mint); } catch { continue; }
    if (await targetHit(order, intel)) { await executeOrder(client, store, order); return; }
  }
  store.save();
}
  ...
  for (const setup of setups) {
    ...
    for (const profile of fresh) {
      ...
      try {
        ...
        store.save();
        await notify(client, store, `🎯 **V4 launch-profile snipe #${setup.id}** · **${intel.symbol}** · ${setup.mode.toUpperCase()} ${setup.solAmount} SOL\nMint: \`${mint}\`${r?.signature ? `\nhttps://solscan.io/tx/${r.signature}` : ''}`);
        return;
      } catch (e) {
        setup.lastError = String(e.message || e).slice(0, 400); store.save();
      }
    }
  }
  store.save();
...
export function startV4Strategies(client, store) {
  ...
  setInterval(async () => {
    if (orderBusy) return;
    orderBusy = true;
    try { await processOrders(client, store); }
    catch (e) { console.error('V4 strategy monitor:', e.message); }
  ...
  setInterval(async () => {
    ...
    sniperBusy = true;
    try { await processSnipers(client, store); }
    catch (e) { console.error('V4 launch scanner:', e.message); }
    finally { sniperBusy = false; }
  }, config.launchPollMs);
}
